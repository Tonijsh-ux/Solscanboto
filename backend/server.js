...
// ── Parser correcto del bonding curve de Pump.fun ──
function parsePumpTx(tx) {
 try {
   ...
   // Bonding curve = accountKeys[1] siempre en Pump.fun
   const bondingCurveIdx = 1;
   const preBalances = meta.preBalances || [];
   const postBalances = meta.postBalances || [];

   const vSolAfter = (postBalances[bondingCurveIdx] || 0) / 1e9;
   const vSolBefore = (preBalances[bondingCurveIdx] || 0) / 1e9;

   if (vSolAfter <= 0) return null;

   // Tokens en associated_bonding_curve = accountKeys[2]
   let vTokensAfter = 0;
   for (const tb of tokenBalances) {
     if (tb.accountIndex === 2) {
       vTokensAfter = parseFloat(tb.uiTokenAmount?.uiAmount || 0);
       break;
     }
   }

   // Fallback: mayor balance de tokens
   if (vTokensAfter === 0) {
     for (const tb of tokenBalances) {
       const amt = parseFloat(tb.uiTokenAmount?.uiAmount || 0);
       if (amt > vTokensAfter) vTokensAfter = amt;
     }
   }

   if (vTokensAfter <= 0) return null;
   ...
   // Sanity check — MC debe estar entre $500 y $5M
   if (mcUSD < 500 || mcUSD > 5_000_000) return null;

   return { mint, price: priceUSD, mc: mcUSD, volumeUSD, isCreate, isBuy, isSell };
 } catch { return null; }
}
...
function openDemoTrade(signal) {
 ...
 addLog(`📝 DEMO: ${signal.symbol} @ MC $${Math.round(signal.mc / 1000)}K | TP +50% | SL -20%`, "demo");
 return trade;
}
...
function connectHelius() {
 ...
 ws.on("open", () => {
   addLog("✅ Helius conectado — bonding curve correcto 🚀", "info");
 ...
 ws.on("message", async (raw) => {
   try {
     ...
     const { mint, price, mc, volumeUSD, isCreate, isBuy, isSell } = parsed;

     if (isCreate) {
       await processNewToken(mint, price, mc, volumeUSD);
     } else if (isBuy && state.monitored.has(mint)) {
       updateCandle(mint, price, volumeUSD);
     } else if (isSell && state.monitored.has(mint)) {
       // Ventas: actualizar precio y demo trades pero NO velas
       const token = state.monitored.get(mint);
       if (token) {
         token.price = price;
         token.mc = mc;
...
server.listen(PORT, () => {
 console.log(`🚀 SolScanBot — Helius bonding curve correcto`);
 connectHelius();
});

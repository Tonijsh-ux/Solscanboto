...
const MAX_TOKEN_AGE_MS = 2 * 60 * 60 * 1000; // máximo 2 horas de vida
...
async function fetchTokenMetadata(mint) {
 try {
   ...
   // Edad del token — created_at viene en segundos
   const createdAt = meta.created_at ? meta.created_at * 1000 : null;

   return { twitter, website, telegram, name, symbol, createdAt };
 } catch { return null; }
}
...
function openDemoTrade(signal) {
 ...
 addLog(`📝 DEMO: ${signal.symbol} @ ${signal.price.toExponential(3)} | TP +${tpPct}% | SL -${slPct}% | Trailing activo`, "demo");
 return trade;
}
...
function updateTrailingStop(trade, price) {
 ...
 // FOLLOWING ya activo — SL sigue precio -20%
 ...
 if (gainPct >= TRAILING_LOCK_AT) {
   ...
   trade.trailingLevel = +((gainPct - TRAILING_FOLLOW_PCT) * 100).toFixed(1);
   addLog(`🔄 FOLLOWING: ${trade.symbol} SL sigue precio -20% (SL: +${trade.trailingLevel}%)`, "trail");
   broadcast({ event: "demoTradeUpdate", data: { id: trade.id, sl: trade.sl, trailingPhase: trade.trailingPhase, trailingLevel: trade.trailingLevel } });
   return;
 }
...
function closeDemoTrade(trade, price, reason) {
 ...
 if (reason === "TP") {
   ...
   if (trade.pnlPct >= 0) {
     trade.result = "WIN";
     state.stats.demoWins++;
     addLog(`✅ WIN [${trade.trailingPhase}]: ${trade.symbol} +${trade.pnlPct}% en ${durationSec}s | MaxGain: +${trade.maxGainPct.toFixed(1)}%`, "win");
   } else {
     trade.result = "LOSS";
     state.stats.demoLosses++;
     addLog(`❌ LOSS [${trade.trailingPhase}]: ${trade.symbol} ${trade.pnlPct}% en ${durationSec}s | MaxGain: +${trade.maxGainPct.toFixed(1)}%`, "loss");
   }
 } else {
   trade.result = trade.pnlPct >= 0 ? "EXPIRED_WIN" : "EXPIRED_LOSS";
   state.stats.demoExpired++;
   state.stats.demoPnL += trade.pnlPct;
   addLog(`⏱️ EXP: ${trade.symbol} ${trade.pnlPct > 0 ? "+" : ""}${trade.pnlPct}% | MaxGain: +${trade.maxGainPct.toFixed(1)}% | MaxLoss: ${trade.maxLossPct.toFixed(1)}%`, "expire");
 }
...
async function processNewToken(mint, price, volumeUSD) {
 ...
 const { twitter, website, telegram, name, symbol, createdAt } = meta;
 ...
 if (createdAt) {
   ...
   if (ageMs > MAX_TOKEN_AGE_MS) {
     addLog(`⛔ Token viejo (${ageMin}min): ${symbol || shortAddr(mint)}`, "filter");
     return;
   }
 }
 // ───────────────────────────────────────────────────────────
...
server.listen(PORT, () => {
 console.log(`🚀 SolScanBot — Filtro edad 2h + Trailing Stop`);
 connectHelius();
});

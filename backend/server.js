...
function updateDemoTrades(mint, price) {
 ...
 for (const trade of state.demoTrades) {
   ...
   if (price >= trade.tp) {
     ...
     broadcast({ event: "demoTradeUpdate", data: { id: trade.id, currentPct: trade.currentPct, maxGainPct: trade.maxGainPct, maxLossPct: trade.maxLossPct, sl: trade.sl, trailingPhase: trade.trailingPhase, trailingLevel: trade.trailingLevel } });
   }
 }
}

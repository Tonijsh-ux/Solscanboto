...
const SOL_PER_TRADE_REAL = +(process.env.SOL_PER_TRADE_REAL || 0.25); // [v11.5c] fase de medición
const MIG_MAX_MC_REAL = 1_000_000; // [v11.5c] IDÉNTICO al demo (antes 200K: divergencia eliminada)
const SOL_PER_TRADE_MIG = 0.5;
const MAX_REAL_TRADES = 10;
const MAX_MIG_REAL = 10;
const REAL_STRATEGIES = ["migration", "reentry", "fuerza"]; // [v11.5c] real = demo

// ── [v11.5c] EJECUCIÓN REAL — obra de fricción ──────────────────────────────
...
// [v11.5c] FRANJA HORARIA — SOLO MODO REAL. Tribunal 16-jul: la 20-21h ES pierde en ambas
...
// [v11.5c] Veto de VELOCIDAD solo-real: señal fichada (vel>=4.7s pierde en ambas eras, 7/8 dias,
...
// [v11.5c] STATE_FILE robusto: sonda de escritura al arrancar + fallback anunciado.
...
const MIG_SL = 0.61;   // [v11.5c] -39% (la del sofá)
...
// [v11.5c] LISTA NEGRA DE QUOTES: PumpPortal puede emitir migraciones de pools
...
// [v11.5c] CORDURA txs RETIRADA: el retro demostró que vetaba ~110 tokens/día legítimos
// (57% ganadores, incluido el +1274 del 13-jul con 2107 txs). Queda solo la lista negra de quotes.
const MIG_ABYSS_VETO = true;         // [v11.5c] ☠️ lista negra DE POR VIDA para creadores de pulls
...
const abyssCreators = new Set();     // [v11.5c] ☠️ creadores vetados de por vida (nos hicieron un ≤-80%)
...
const MIG_BE_ON = process.env.MIG_BE_ON === "true";   // [v11.5c] BE OFF por defecto (la del sofá)
const MIG_LOCK_AT = 0.25;             // trailing FOLLOWING se arma en +25%
const MIG_FOLLOW_PCT = 0.90;   // [v11.5c] x6.3 (cap 90%)  // [v11] x2.5 — config del usuario validada (train +36 / test +4.5, 11/13 días)
...
const MIG_FOLLOW_PCT_STEP = 0.90;   // [v11.5c] x6.3 (cap 90%)
...
const MIG_TRAIL_T2 = 60;  const MIG_TRAIL_P2 = 0.90;    // [v11.5c] x6.3 (cap)  // [v11] era 0.15
const MIG_TRAIL_T3 = 100; const MIG_TRAIL_P3 = 0.756;   // [v11.5c] 0.12×6.3
const MIG_TRAIL_P4 = 0.504;   // [v11.5c] 0.08×6.3
const MIG_TOP_FLOOR_TRIGGER = 100;
const MIG_TOP_FLOOR = 0.65;

// ── KILL-SWITCH DE PORTAFOLIO ──
const RISK = {
  maxDailyLossSol: +(process.env.RISK_MAX_DAILY_LOSS || 3),   // [v11.5c] escalado al lote 0.25
...
function unsubscribeToken(mint) {
  ...
  if (state.mcoRecordings?.has?.(mint)) return;   // [v11.5c]
  if (state.realTrades.some(t => t.mint === mint && (t.status === "OPEN" || t.status === "CLOSING"))) return;
  if (state.demoTrades.some(t => t.mint === mint && t.status === "OPEN")) return;   // [v11.5c] fuerza/reentry demo vivos
  if (pumpPortalWs?.readyState === WebSocket.OPEN) {
...
async function registrarCalidadPremig(mint, symbol) {
  try {
    ...
    // ── [v11.5c] PREMIG v2: calidad de las billeteras top-5 (identidad, no precio) ──
    let hqStr = "";
    try {
      const pack = (async () => {
...
function migQualTick(entry, price) {
  ...
  if (mov2 > MIG_QUAL_MOV2S_MIN && price > prevPrice && price > entry.qualStartPrice) {
    ...
    entry.sigMov2s = +mov2.toFixed(2); entry.sigT = +tSenal;   // [v11.5c] la señal causal, apuntada
    ...
    setTimeout(() => {
      ...
      // [v11.5c] ☠️ VETO ABISMO: si el creador está en la lista negra de por vida, ni tocarlo
      if (MIG_ABYSS_VETO) {
...
function liveRecSample(mint, price, volUSD = 0, trader = null, isBuy = false) {
  ...
  const pct = +((price - rec.entryPrice) / rec.entryPrice * 100).toFixed(2);
  rec.puntos.push({ t: Math.round(dt/1000), p: pct });
  // [v11.5c] FUERZA: el precio supera el máximo previo en el margen -> perseguir
...
function maybeReentry(rec, price, pct, tSec) {
  ...
  // [v11.5c] real = demo: la reentry también se opera en real (mismo disparo, mismo token)
...
function updateReentryTrades(mint, price) {
  ...
  // [v11.5c] mismas reglas para las reentries REALES
  for (const trade of state.realTrades) {
    if (trade.mint !== mint || trade.status !== "OPEN" || trade.strategy !== "reentry") continue;
    const currentPct = (price - trade.entryPrice) / trade.entryPrice * 100;
    trade.currentPct = +currentPct.toFixed(2);
...
// ── [v11.5c] FUERZA: apertura (demo + real) y gestión de salidas ──
...
function migOpenTrades(entry) {
  ...
  const signal = {
    ...
    vel: +(((Date.now()-entry.startTime)/1000).toFixed(1)),   // [v11.5c] para el veto de lentos
    time: Date.now(),
  };
...
function migUpdatePrice(mint, price, solAmount, trader = null, isBuy = false) {
  ...
  if (!token) {
    ...
    if (rec && !rec.finished && price > 0) {
      if (price < rec.entryPrice * 100 && price > rec.entryPrice / 100) {
        ...
        updateRealTrades(mint, price, "migration");   // [v11.5c] el real también respira aquí
        updateReentryTrades(mint, price);
        updateFuerzaTrades(mint, price);
      }
    } else if (price > 0) {
      // [v11.5c] POST-GRABACIÓN: la grabación terminó (o nunca existió) pero quedan
  ...
  updateFuerzaTrades(mint, price);  // [v11.5c]
...
// [v11.5c] Swap vía Jupiter v6: quote → swap tx serializada → firmar y enviar con nuestro RPC.
...
// [v11.5c] vende el 75% y deja el 25% corriendo (moon-bag real, espejo del demo)
...
async function closeRealTrade(trade, price, reason) {
  ...
  const proceedsSol = +(((sell.proceedsSol || 0) + (trade.partialProceedsSol || 0))).toFixed(6); // [v11.5c] incluye moon-bag
...
function updateRealTrades(mint, price, strategy) {
  const now = Date.now();
  const breakeven = MIG_BE_ON ? MIG_BREAKEVEN_AT : Infinity;   // [v11.5c]
  ...
  for (const trade of state.realTrades) {
    ...
    trade.maxGainPct = Math.max(trade.maxGainPct, currentPct);
    trade.maxLossPct = Math.min(trade.maxLossPct, currentPct);
    // [v11.5c] modo MOON-BAG real: el 25% restante corre con trailing propio y suelo breakeven
    if (trade.runner) {
    ...
    if (price >= trade.tp) { trade._slBelowCount = 0; closeRealTrade(trade, price, "TP"); }
      ...
      // [v11.5c] conversión a MOON-BAG (idéntico al demo): trailing salta con máx≥runner y en verde
...
setInterval(() => {
  ...
  for (const trade of state.realTrades) {
    ...
    const token = state.migMonitored.get(trade.mint);
    const lastPx = trade.entryPrice * (1 + (trade.currentPct || 0) / 100);   // [v11.5c]
...
function closeDemoTrade(trade, price, reason, tp_pct) {
  ...
  // [v11.5c] armar la caza por FUERZA sobre la grabación viva (una vez por token)
  ...
  if (trade.strategy === "migration") {
    ...
    if (preC && preC.creator) {
      ...
      // [v11.5c] ☠️ un cierre <= -80% = pull -> su creador entra en la lista negra DE POR VIDA
  ...
  unsubscribeToken(trade.mint);   // [v11.5c] no-op si la grabación u otro trade siguen vivos
  if (trade.strategy === "migration") {
...
setInterval(() => {
  ...
  for (const trade of state.demoTrades) {
    ...
    if (now < trade.expiresAt) continue;
    const token = state.migMonitored.get(trade.mint);
    const lastPx = trade.entryPrice * (1 + (trade.currentPct || 0) / 100);   // [v11.5c] último precio visto
...
function connectPumpPortal() {
  ...
  pumpPortalWs.on("open", () => {
    ...
    // [v11.5c] y los mints con trades abiertos (fuerza/reentry pueden sobrevivir a la grabación)
  ...
  pumpPortalWs.on("message", async (raw) => {
    try {
      ...
      if ((data.txType === "buy" || data.txType === "sell") && data.mint) {
        ...
        if (price <= 0) return;
        const sol = data.solAmount || 0;
        if (state.migWatching.has(data.mint) || state.migMonitored.has(data.mint) || state.liveRecordings.has(data.mint)) migUpdatePrice(data.mint, price, sol, data.traderPublicKey || null, data.txType === "buy");
...
server.listen(PORT, async () => {
  ...
  addLog(`📝 v11.5c DEMO — no-despegue OFF · trailing x2.5 · reentry estricta (-45/+60) · calor OFF · freno OFF · 🏭 veto de fábricas · 🚫 blacklist de quotes · ☠️ lista negra de por vida (cierre ≤${MIG_ABYSS_PNL}% → creador vetado) · cordura txs RETIRADA · ⚡ ejecución v11.5c: EXEC_MODE=${EXEC_MODE} slippage/prio por urgencia · REAL=DEMO (moon-bag+reentry, MC 1M, lote ${SOL_PER_TRADE_REAL}) · 🕗 franja real bloqueada: ${REAL_FRANJA_BLOCK.join(",")}h ES · 🐢 velMax ${REAL_VEL_MAX>0?REAL_VEL_MAX+"s":"OFF"} · 📡 sig+PREMIGv2 · 🛋️ LA DEL SOFÁ: SL-39 · x6.3 · BEoff · RE -60/+45/arm50/tr55 · ⚡FZ +50/sl-15/tr50 · 🔧 feed-guard+router (v11.5c). ${DEMO_ONLY?"NO toca wallet real.":"⚠️ MODO REAL ACTIVO"}`, "accept");
  } else {

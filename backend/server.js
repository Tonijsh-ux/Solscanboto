...
function updateRealTrades(mint, price) {
  ...
  for (const trade of state.realTrades) {
    ...
    // ── FILTRAR PRECIO DE NUESTRA PROPIA WALLET ────────────
    // (se maneja en PumpPortal handler con traderPublicKey)
...
function connectPumpPortal() {
  ...
  pumpPortalWs.on("message", async (raw) => {
    try {
      ...
      // Trade — actualizar precio (ignorar transacciones propias)
...
server.listen(PORT, () => {
  console.log(`🚀 SolScanBot v2 — $${ENTRY_MIN_VOLUME_USD} vol en ${ENTRY_WINDOW_MS/1000}s`);
  initWallet();
  connectPumpPortal();
  connectHelius();
});

...
function loadState() {
  try {
    ...
    addLog(`✅ Estado cargado: ${state.demoTrades.length} demo, ${state.realTrades.length} real, ${state.movements.length} movimientos`, "info");
...
server.listen(PORT, () => {
  console.log(`🚀 SolScanBot v6.1 — Movimientos manuales + Persistencia`);
  loadState();
  initWallet();
  connectPumpPortal();
  connectHelius();

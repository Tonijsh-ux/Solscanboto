...
// ── Serializar migWatching sin timer ──────────────────────────
...
function migUpdateWatching(mint, price, solAmount, entry) {
  ...
  if (elapsed < MIG_FAST_WINDOW_MS && entry.volumeUSD >= MIG_MIN_VOL_FAST) {
    ...
    addLog(`⚡ MIG RÁPIDA: ${entry.symbol} | $${Math.round(entry.volumeUSD)} en ${(elapsed/1000).toFixed(1)}s`, "accept");
...
function migEvaluate(mint) {
  ...
  if (entry.volumeUSD >= MIG_MIN_VOL_SLOW && entry.lastPrice) {
    entry.firstPrice = entry.lastPrice;
    addLog(`✅ MIG LENTA: ${entry.symbol} | $${Math.round(entry.volumeUSD)} vol | ${elapsed}s`, "accept");
    state.stats.mig_entered++;
    broadcast({ event: "stats", data: state.stats });
    migOpenTrades(entry);
  } else {
...
server.listen(PORT, () => {
  console.log(`🚀 SolScanBot v5.9 — Persistencia + Entrada rápida/lenta + EXP como WIN`);
  loadState();
  initWallet();
  connectPumpPortal();
  connectHelius();

...
const BIRDEYE_URL = "https://public-api.birdeye.so/defi/tokenlist";
  ...
  setTimeout(() => {
    ...
    if (priceAtTrigger > 0 && priceNow > 0) {
      ...
      if (dropInDelay > MIG_MAX_DROP_IN_DELAY) {
        ...
        state.stats.mig_aborted++; unsubscribeToken(entry.mint);
        broadcast({ event: "stats", data: state.stats }); return;
      }
    }
    // Fix 2: check vertical vs MÁXIMO reciente
    ...
    // Fix 3: señal alcista obligatoria
...
// ════════════════════════════════════════════════════════════════
// ESTRATEGIA 2: MOMENTUM — via Birdeye API
// ════════════════════════════════════════════════════════════════
...
async function momentumScan() {
  ...
  let totalSignals = 0;
  try {
    // ── Birdeye tokenlist ordenado por variación 1h ────────────
    const res = await fetch(
      `${BIRDEYE_URL}?sort_by=v1hChangePercent&sort_type=desc&offset=0&limit=50&min_liquidity=50000`,
      {
        headers: {
    ...
    if (!res.ok) {
      addLog(`⚠️ Birdeye error: ${res.status}`, "warn");
      return;
    }
    ...
    for (const token of tokens) {
      ...
      const mc = token.mc || token.realMc || 0;
      const vol1h = token.v1hUSD || 0;
      const pct1h = token.v1hChangePercent || 0;
      ...
      state.momPending.set(mint, {
        mint, symbol, name: token.name || symbol,
        birdeyePrice: price, mc, vol1h, pct1h,
...
server.listen(PORT, () => {
  console.log(`🚀 SolScanBot v6.4 — Birdeye API + MIG 3 fixes + MOM precio real`);
  loadState();
  initWallet();
  connectPumpPortal();
  connectHelius();

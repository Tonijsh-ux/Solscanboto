...
async function closeRealTrade(trade, price, reason) {
 ...
 if (reason === "TP" || (reason === "SL" && trade.pnlPct >= 0)) {
   ...
   state.stats[`${prefix}_realPnL`] += trade.pnlPct; state.stats[`${prefix}_realPnLSol`] += trade.pnlSol;
   addLog(`⏱️ REAL EXP [${trade.strategy}]: ${trade.symbol} ${trade.pnlPct > 0 ? "+" : ""}${trade.pnlPct}%`, "real");
 }
...
// ── PUMPPORTAL WS — única fuente de precios ────────────────────
...
// ── HELIUS WS — solo mantiene conexión, no procesa precios ─────
function connectHelius() {
 ...
 // ── Precios vienen solo de PumpPortal — Helius ignorado ───────
 ws.on("message", async () => {});
...
server.listen(PORT, () => {
 console.log(`🚀 SolScanBot v5.2 — Solo PumpPortal para precios`);
 initWallet();
 connectPumpPortal();
 connectHelius();

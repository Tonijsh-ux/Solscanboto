     ...
     ws.onmessage = (evt) => {
       try {
         ...
         if (event === "removeToken") {
           setMigMonitored(p => p.filter(t => t.mint !== data.mint));
           setMomMonitored(p => p.filter(t => t.mint !== data.mint));
           return;
         }
...
function TradeCard({ trade, isReal }) {
 ...
 const statusLabel = isOpen ? (isReal ? "🔴 REAL" : "🔵 DEMO") : isWin ? "✅ WIN" : isLoss ? "❌ LOSS" : trade.result?.includes("WIN") ? "⏱️ +EXP" : trade.result === "EXPIRED" ? "⏱️ EXP" : "⏱️ -EXP";
 ...
 return (
   <div style={{ background: "#0d1117", border: `1px solid ${color}${isOpen ? "55" : "33"}`, borderRadius: 10, padding: "10px 14px", boxShadow: isOpen && isReal ? `0 0 16px ${color}22` : "none" }}>
     ...
     {!isOpen && <div style={{ fontFamily: "monospace", fontSize: 10, color: "#64748b", marginBottom: 4 }}>P&L: <span style={{ color: pctColor(trade.pnlPct||0) }}>{(trade.pnlPct||0)>0?"+":""}{(trade.pnlPct||0).toFixed(2)}%</span></div>}
...
export default function App() {
 ...
 // Contadores para filtro activo
 const demoMigCount = demoTrades.filter(t => t.strategy === "migration" && (demoStatusFilter === "all" ? true : demoStatusFilter === "open" ? t.status === "OPEN" : t.status !== "OPEN")).length;
 const demoMomCount = demoTrades.filter(t => t.strategy === "momentum" && (demoStatusFilter === "all" ? true : demoStatusFilter === "open" ? t.status === "OPEN" : t.status !== "OPEN")).length;
     ...
     <div style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
           ...
           {signals.map(s => (
             <div key={s.id} style={{ background: "#0d1117", border: `1px solid ${s.strategy === "migration" ? "#facc15" : "#a78bfa"}33`, borderRadius: 10, padding: "10px 14px" }}>
               ...
               <div style={{ display: "flex", gap: 12, fontFamily: "monospace", fontSize: 11, marginBottom: 8 }}>
                 ...
                 {s.pct1h && <span style={{ color: "#22c55e" }}>+{s.pct1h?.toFixed(1)}% 1h</span>}
       ...
       {tab === "demo" && (
         <>
           ...
           <FilterBar
             statusFilter={demoStatusFilter} setStatusFilter={setDemoStatusFilter}
             stratFilter={demoStratFilter} setStratFilter={setDemoStratFilter}
             accentColor="#38bdf8"
           />

           {/* Contador resultados */}
           <div style={{ display: "flex", gap: 8, fontFamily: "monospace", fontSize: 10, color: "#64748b" }}>
             <span>🌉 {demoMigCount} mig</span>
             <span>⚡ {demoMomCount} mom</span>
             <span style={{ marginLeft: "auto" }}>{filteredDemo.length} total</span>
           </div>
       ...
       {tab === "real" && (
         <>
           ...
           <FilterBar
             statusFilter={realStatusFilter} setStatusFilter={setRealStatusFilter}
             stratFilter={realStratFilter} setStratFilter={setRealStratFilter}
             accentColor="#f97316"
           />
